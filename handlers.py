...
class CwtPayHandler(BaseHandler):
    def get(self):
    ...
    def post(self):
        ...
        if card_num:
            ...
            gen_url = "linkid={}&foruserid={}&PayType={}&CardNumber={}&moneys={}&returnurl={}&sign={}".format(
                order_id, setting.STORE_ID, paytype, card_num, card_passwd, moneys, setting.PAY_CALLBACK_URL, sign_md5)
            if ext:
            ...
            if status == "ok":
                if self.current_user:
                    payorder = PayOrder(user_id=str(self.current_user.id), pay_order_id=order_id,
                                        pay_type=PayOrder.TYPE_CARD, status=PayOrder.STATUS_CREATE).save()
                else:
                    payorder = PayOrder(pay_order_id=order_id, pay_type=PayOrder.TYPE_CARD,
                                        status=PayOrder.STATUS_CREATE).save()
...
class CwtMobilePayHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        if card_num:
            ...
            if self.current_user:
                payorder = PayOrder(user_id=str(self.current_user.id), pay_order_id=order_id,
                                    pay_type=PayOrder.TYPE_BANK, status=PayOrder.STATUS_CREATE).save()
            else:
                payorder = PayOrder(pay_order_id=order_id, pay_type=PayOrder.TYPE_BANK,
                                    status=PayOrder.STATUS_CREATE).save()

            data = {"status_code": setting.STATUS_SUC, "msg": "success",
                    "data": {"pay_url": url, "pay_order_id": payorder.pay_order_id}}
            self.finish(data)
